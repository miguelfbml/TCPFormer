...
def load_mpi_test_frames(sequence_name, num_frames=50):
    ...
    for root in mpi_roots:
       stru: System: The ground truth connections in the provided MediaPipe script (`estimate_3d_pose_mediapipe.py`) are correct, but the MediaPipe prediction connections are not displaying properly. The user has provided a list of MediaPipe keypoints (0-32) and wants the connections to be meaningful and comparable to the ground truth connections, which are defined in the MPI-INF-3DHP skeleton structure. The issue stems from incorrect connections for MediaPipe predictions, as not all MediaPipe keypoints have direct or meaningful mappings to the MPI-INF-3DHP joints, and some connections may be invalid or irrelevant for comparison.

The MPI-INF-3DHP skeleton has 17 joints, and the ground truth connections are correctly mapped using the `H36M_TO_MPI` mapping. The MediaPipe script needs to use connections that align with these ground truth connections, limited to the joints that MediaPipe can reliably estimate based on the provided `mp_to_mpi_mapping`. The goal is to update the `mp_connections` list to include only meaningful connections that correspond to the ground truth skeleton, ensuring visual consistency and meaningful comparison.

Below is the revised script with corrected MediaPipe connections, maintaining the same `artifact_id` as the previous version since this is an update. The changes focus on defining `mp_connections` to include only valid and meaningful connections based on the MediaPipe keypoints and their mapping to MPI-INF-3DHP joints.

<xaiArtifact artifact_id="760c2bcc-f8c8-446f-bb38-284db4a42097" artifact_version_id="a06f5c43-9fc6-493d-90cc-66a8a167200f" title="estimate_3d_pose_mediapipe.py" contentType="text/python">
import argparse
import os
import cv2
import numpy as np
