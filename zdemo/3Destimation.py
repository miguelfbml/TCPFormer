...
def main():
    ...
    # Setup 3D plot only if visualization is enabled
        ...
        while True:
            ...
            if (results and len(results) > 0 and 
                ...
                if args.show_fps:
                    if len(tcpformer_inference_times) > 0:
                        if not args.no_viz and len(viz_update_times) > 0:
                            print(f"Buffer: {len(pose_buffer)}/{n_frames} | YOLO: {current_yolo_fps:.1f} FPS | TCPFormer: {current_tcpformer_fps:.1f} FPS | 3D Viz: {current_viz_fps:.1f} FPS", end='\r')
                        else:
                            print(f"Buffer: {len(pose_buffer)}/{n_frames} | YOLO: {current_yolo_fps:.1f} FPS | TCPFormer: {current_tcpformer_fps:.1f} FPS", end='\r')
                    else:
                        print(f"Buffer: {len(pose_buffer)}/{n_frames} | YOLO: {current_yolo_fps:.1f} FPS", end='\r')
                
                # Run TCPFormer prediction when we have exactly 27 frames
                if len(pose_buffer) == n_frames:
                    prediction_count += 1
                    ...
                    # Update 3D visualization only if enabled and measure time
                    if not args.no_viz:
                        viz_start = time.time()
                ...
                if args.show_fps:
                    if len(tcpformer_inference_times) > 0:
                        if not args.no_viz and len(viz_update_times) > 0:
                            print(f"No person detected | Frame: {frame_count} | YOLO: {current_yolo_fps:.1f} FPS | TCPFormer: {current_tcpformer_fps:.1f} FPS | 3D Viz: {current_viz_fps:.1f} FPS", end='\r')
                        else:
                            print(f"No person detected | Frame: {frame_count} | YOLO: {current_yolo_fps:.1f} FPS | TCPFormer: {current_tcpformer_fps:.1f} FPS", end='\r')
                    else:
                        print(f"No person detected | Frame: {frame_count} | YOLO: {current_yolo_fps:.1f} FPS", end='\r')
